...
import { hasGameEnded, isLiveGame, gameNotStarted } from '../../../../utils/utilFunctions.js';
...
class FixtureByIdTTL extends TtlStrategy {
	async getInSeconds({ data: { response } }) {
		...
		let ttl = 0;
		const now = new Date();
		console.log(now, this.fixtureDate, fixture);
		if (
			isLiveGame(fixture) ||
...
async function handleFutureFixture(fixtureDate, match) {
	...
	if (await fixturesHelper.hasCoverage(leagueId)) {
		...
		// se for < 30 minutos cache de 15 segundos até receber informaçao dos lineups
		if (timeUntilMatchStart < 1800 && lineups.length === 0) {
			// > 30min
			return 15;
		} else {
			if (isToday(fixtureDate)) {
				...
				myDate = new Date(
					fixtureDate.setMinutes(fixtureDate.getMinutes() - 30)
				);
			} else {
